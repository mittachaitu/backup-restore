...
type HookLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Hook, err error)
...
// List lists all Hooks in the indexer.
func (s *hookLister) List(selector labels.Selector) (ret []*v1.Hook, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Hook))
	})
	return ret, err
}
...
// List lists all Hooks in the indexer for a given namespace.
func (s hookNamespaceLister) List(selector labels.Selector) (ret []*v1.Hook, err error) {
...
// Get retrieves the Hook from the indexer for a given namespace and name.
func (s hookNamespaceLister) Get(name string) (*v1.Hook, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("hook"), name)
	}
	return obj.(*v1.Hook), nil
}
